...
/*
https://scanftree.com/Data_Structure/huffman-code
*/
...
struct M_Heap* createM_Heap(unsigned space) {
  ...
  M_Heap->space = space;
  M_Heap->array =
      (struct MH_Node**)malloc(M_Heap->space * sizeof(struct MH_Node*));
  return M_Heap;
}
...
void M_Heapify(struct M_Heap* M_Heap, int idx) {
  ...
  if (l < M_Heap->size &&
      M_Heap->array[l]->frequency < M_Heap->array[smallest]->frequency)
    smallest = l;

  if (r < M_Heap->size &&
      M_Heap->array[r]->frequency < M_Heap->array[smallest]->frequency)
    smallest = r;
...
int isSizeOne(struct M_Heap* M_Heap) { return (M_Heap->size == 1); }
...
void buildM_Heap(struct M_Heap* M_Heap) {
  int n = M_Heap->size - 1;
  int i;
  for (i = (n - 1) / 2; i >= 0; --i) M_Heapify(M_Heap, i);
}
...
int isLeaf(struct MH_Node* root) { return !(root->l) && !(root->r); }

struct M_Heap* createAndBuildM_Heap(char character[], int frequency[],
                                    int size) {
  int i;
