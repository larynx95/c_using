...
#include <stdio.h>
void Adjust(int Heap_of_Numbers[],
            int i) /*Function to arrange the elements in the heap*/
{
  int j;
  int copy;
  int Number;
...
void Make_Heap(int heap[]) {
  ...
  for (i = Number_of_Elements / 2; i >= 1; i--) Adjust(heap, i);
}
int main() {
  int heap[30];
  ...
  printf(
      "nEnter the members of the array one by one:"); /* Asking for the elements
